...
function Set_review(){
  ...
  useEffect(()=>{
    if(reviewSetting){
      setContents(reviewSetting.title);
      if(reviewSetting.img !== null){
        setUpload(reviewSetting.img);
      }
    }
  }, [reviewSetting])
  ...
  if(reviewSetting){
    return(
      <form id="reviewSetting" className="forms" data-whats="admin_review" data-idfy="admin_setting">
          ...
          <div className="inner_con">
            <div className="input_con img_input">
              <label>
                ...
                <div className="upload_img">
                  {upload_logo_img == '' ? <img src="img/basic/notice/notice_base.jpg"></img> : upload_logo_img !== '' && reviewSetting.img == '' ? <img src={upload_logo_img}></img> : <img src={"data:image;base64,"+upload_logo_img} alt="이미지 경로를 확인해주세요"></img>}
                </div>
              </label>
              {
            ...
            <div className="opa_con">
              <div className="opacity_con">
                <input type="color" name="bgcolor" className="clr_input" defaultValue={reviewSetting.bgcolor}></input>
                <div className="range_con">
                  <input type="range" name="transparent" className="opacity_input" min={0} max={100} defaultValue={reviewSetting.transparent} onChange={(e)=>{
                    let val = e.target.value;
                    $(".opa span").text(val+"%")
                  }}></input>
                  <p className="opa"><span>{reviewSetting.transparent}%</span>배경색 투명도</p>
                </div>
              </div>
              <div className="ex_con">
                <p className="row_desc">결과예시)</p>
          ...
          <div className="title_con">
            ...
            <div className="check_con">
              ...
              <input className="input_check_hidden" name="title_show" type="hidden" value='N'></input>
                <input className="check_menu input_check" name="title_show" type="checkbox" value='Y' defaultChecked={reviewSetting.title_show == "Y" ? true : false}></input>
